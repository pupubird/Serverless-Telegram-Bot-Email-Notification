...
BLACKLIST = s3.Object(os.environ['BUCKETNAME'], os.environ['BLACKLIST']).get()[
...
def main():
    ...
    print("Email amount:", len(mail_ids), "-",
          LAST_ID, "=", len(mail_ids) - LAST_ID)
    for i in range(len(mail_ids), 1, -1):
        if (abs(len(mail_ids) - i) >= MAX_DEPTH and MAX_DEPTH != 0) or i == LAST_ID or LAST_ID > len(mail_ids):
            break
        ...
        for response_part in data:
            ...
            if isinstance(response_part, tuple):
                ...
                mail_from = re.sub(
                    '<.*>', "", mail_from).replace("\"", "").replace("_", "").replace("*", "")
                mail_subject = message['subject']
                mail_subject = make_header(decode_header(
                    mail_subject))
                mail_subject = f"{mail_subject}".replace(
                    "_", "").replace("*", "")
    ...
    string = "All latest emails notification sent." if LAST_ID != len(
        mail_ids) else "No new emails"
    print(string)
    return True
